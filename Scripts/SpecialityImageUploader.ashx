<%@ WebHandler Language="C#" Class="SpecialityImageUploader" %>
...
public class SpecialityImageUploader : IHttpHandler {
    
    public void ProcessRequest (HttpContext context) {
         var response = new JsonResponse() { IsSuccess = false, Message = "File Not uploaded Successfully" };
        ...
        if (!string.IsNullOrEmpty(id) && !string.IsNullOrEmpty(fnames))
        {
            ...
            var specialityImages = new SpecialityManager().GetSpecialityImageById(id);

            var images = specialityImages.Split(' ');
            ...
            var uploaded = new SpecialityManager().UpdateSpecialityImageById(id, string.Join(" ", imagesToAdd));
            if (uploaded)
            {
                try
                {
                    foreach (string s in context.Request.Files)
                    {
                        ...
                        var fileName = fnames.Split(' ').ToList().First(x => x.Contains(file.FileName));
