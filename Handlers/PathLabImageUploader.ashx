<%@ WebHandler Language="C#" Class="PathLabImageUploader" %>
...
public class PathLabImageUploader : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        if (!string.IsNullOrEmpty(id) && !string.IsNullOrEmpty(fnames))
        {
            ...
            var pathLabImages = new PathLabManager().GetPathLabImagesById(id);

            var images = pathLabImages.Split(' ');
            ...
            var uploaded = new PathLabManager().UpdatePathLabImageById(id, string.Join(" ", imagesToAdd));
            if (uploaded)
            {
                try
                {
