<%@ WebHandler Language="C#" Class="BloodBankImageUploader" %>
...
public class BloodBankImageUploader : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        if (!string.IsNullOrEmpty(id) && !string.IsNullOrEmpty(fnames))
        {
            ...
            var bloodBankImages = new BloodBankManager().GetBloodBankImagesById(id);

            var images = bloodBankImages.Split(' ');
            ...
            var uploaded = new BloodBankManager().UpdateBloodBankImagesById(id, string.Join(" ", imagesToAdd));
            if (uploaded)
            {
                try
                {
